...
export default function SettingsPage() {
  ...
  const [cycle, setCycle] = useState<InterviewCycle | null>(null);
  ...
  useEffect(() => {
    ...
    db.from("interview_cycles").select("*").eq("id", id).single().then(({ data }) => {
  ...
  if (!cycle) return <div className="text-gray-400 text-sm">Loading…</div>;
